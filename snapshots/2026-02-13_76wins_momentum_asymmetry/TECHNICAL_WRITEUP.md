...
### 6.4 The "Polish" Updates (Ep 200+ Mastery)

After the base Q-SMA was stable, three final tweaks pushed performance from ~60 to ~90 wins and accelerated early learning:

| # | Change | Why | Impact |
|---|--------|-----|--------|
| 15 | **Momentum Filter** (TDA) | Don't kill loops that are *growing* | "Good chaos" allowed; resonance verified |
| 16 | **Negative Flux** | Stagnation should build *avoidance* habit | Agent hates being stuck; faster exits |
| 17 | **Asymmetric Energy (1.15x)** | Losing energy hurts 15% more than gaining | Prevents complacency near goal |

**The Resonance Insight:**
We realized TDA was killing "good loops" — when the agent was swinging higher and higher (building energy), TDA saw a loop and spiked decay.
*Fix:* Added a derivative check. If `d(Energy)/dt > 0.001` inside a loop, it's a **Resonant Swing**. TDA stands down and lets the agent rev the engine.

**The Asymmetry Insight:**
Symmetric reward (`+1` for climb, `-1` for fall) led to complacency at high points.
*Fix:* made falling hurt `1.15x` more. This slight "gravity" keeps the agent scrambling to secure the summit.
...
Performance
    ...
    │  /\                   
    │ /                     ← First exploration burst (Ep 268)
...
|--------|-------|
| Total Episodes | 1000 |
| Successes | 76–93 (varies by run) |
| Mastery Phase (Ep 800-1000) | ~40-60% success rate |
| First Success | **Ep 268** (was 300-400) |
