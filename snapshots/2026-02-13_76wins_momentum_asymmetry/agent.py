...
class QSMA_Agent:
    ...
    def learn(self, state, action, reward, next_state):
        ...
        if energy > 0.1:
            # YANG: High energy = pleasure. Build positive habit.
            self.flux[s, action] += 0.5 
        elif energy < 0.05:
            # YIN: Low energy = pain. Build avoidance habit.
            self.flux[s, action] -= 0.3
            self.flux[s, action] = max(-5.0, self.flux[s, action])  # Floor
            
        self.flux[s, action] *= (1.0 - self.params['decay'])
        self.flux[s, action] = np.clip(self.flux[s, action], -5.0, 5.0)  # Symmetric cap
