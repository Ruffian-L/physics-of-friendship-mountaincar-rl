...
from tda import TopologicalBrain
from agent import QSMA_Agent
import argparse
...
def run_experiment():
    ...
    plot_phase_space(brain, "Final Phase Space")
    ...
    plt.title(f"Mountain Car: Continuous TDA Loop ({success_count} Successes)")
    plt.xlabel("Episode")
    plt.ylabel("Max Position")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig('experiment_results.png')
    print("📊 Results saved to experiment_results.png")
    plt.close()
